...
(function () {
  ...
  /* ---- Skills tabs + horizontal-scroll affordance ---- */
  const tabs = $$('.skills__tab');
  const panels = $$('.skills__panel');
  const tabsEl = $('#skillsTabs');
  const tabsWrap = $('#skillsTabsWrap');
  ...
  function updateTabFades() {
    ...
    tabsWrap.classList.toggle('scrolled', x > 4);
    tabsWrap.classList.toggle('at-end', max <= 4 || x >= max - 4);
    if (skillsHint && max <= 4) skillsHint.classList.add('hide');
  }
  if (tabsEl) {
    tabsEl.addEventListener('scroll', () => { updateTabFades(); if (skillsHint) skillsHint.classList.add('hide'); }, { passive: true });
  ...
  tabs.forEach(tab => {
    tab.addEventListener('click', () => {
      ...
      if (skillsHint) skillsHint.classList.add('hide');
    });
  });
